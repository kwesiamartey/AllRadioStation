<Html>
    
       <body>
       
             <h5 style='text-align: center;'>
                 <img src='https://structuredappsstreaming.win/9186eb0e475eeff5c6856e8ed43eb31e_w200.gif' alt='Girl in a jacket' width='100' height='100'/>
             </h5>
        
             <h5 style='text-align: center;'>
             <!--<video id="video" controls type="application/x-mpegURL"></video>-->
                <audio controls>
                    <source src="https://strm112.1.fm/top40_mobile_mp3?aw_0_req.gdpr=true&listenerid=37ad77cd99b8f8cd7c4d9a5315fe6ddb&awparams=companionAds:true" type="audio/aac">
                   <!--Fallback source for browsers that don't support AAC-->
                    <source src="fallback.mp3" type="audio/mpeg">
                    Your browser does not support the audio element.
                </audio>
              
             </h5>
       </body>
</Html>
<script src='https://cdn.jsdelivr.net/npm/hls.js@latest'></script>
<script>
                      if(Hls.isSupported()) {
                        var video = document.getElementById('video');
                        var hls = new Hls();
                        hls.loadSource('https://d233lioynu5inr.cloudfront.net/out/v1/c45e4e4af26a4d59b5d166c2a9ac2bbc/index.m3u8');
                                      
                        hls.attachMedia(video);
                        hls.on(Hls.Events.MANIFEST_PARSED,function() {
                          video.play();
                      });
                     }
</script>
